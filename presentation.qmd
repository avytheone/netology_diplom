...
subtitle: "(поиск инсайтов, составление рекомендаций стейкхолдерам)"
...
2.  Очистка от выбросов и аномалий.
3.  Разработка предиктивных моделей.
4.  Формирование бизнес-рекомендаций.
...
if not df.empty:
    ...
    def categorize_bmi(bmi):
        ...
        elif bmi < 25: return 'Норма'
        elif bmi < 30: return 'Избыток'
        elif bmi < 35: return 'Ожирение I'
        else: return 'Ожирение II+'
...
if not df.empty:
    ...
    plt.title('Выбросы в AP_HI')
    plt.show()

    plt.figure(figsize=(6, 4))
    sns.boxplot(x=df['height'], color='#808080')
    plt.title('Выбросы в Height')
    plt.show()
```
...
if not df.empty:
    ...
    plt.title('Баланс классов', fontsize=18)
...
Это позволяет:
1. Использовать Accuracy как метрику.
2. Не применять методы оверсемплинга.
:::
:::
...
if not df.empty:
    ...
    # Предположим 1-ж, 2-м
    labels = ['Женщины', 'Мужчины'] if gender_counts.index[0]==1 else ['Мужчины', 'Женщины']
...
1.  **Cardio ↔ Давление** (самая сильная связь).
2.  **Cardio ↔ Возраст**.
3.  **Cardio ↔ Вес/BMI**.
4.  Связь **Smoke ↔ Gender** (мужчины курят чаще).
